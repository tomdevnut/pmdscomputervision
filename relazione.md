...
Durante lo sviluppo sono emerse diverse sfide tecniche. Il problema principale è stato l'elevato carico computazionale richiesto dai modelli di computer vision: sono state adottate soluzioni per ottimizzare le risorse, come l'uso di GPU remote. Per migliorare i tempi di elaborazione, sono state implementate tecniche di preprocessing delle immagini, tra cui la riduzione del numero di frame, la diminuzione delle dimensioni delle immagini e l'applicazione di filtri in bianco e nero.


Il preprocessing prevedeva diverse tecniche per la rimozione dello sfondo al fine di ridurre la complessità e isolare l'oggetto desiderato:

- **HSV (OpenCV)**: Un metodo basato su intervalli di colore che genera maschere binarie in modo rapido ed è efficace con colori uniformi.
- **rembg**: Una libreria Python che utilizza modelli di deep learning come U²-Net per la rimozione automatica dello sfondo.
- **YOLOv8**: Reti neurali convoluzionali che rilevano 80 categorie generiche e restituiscono bounding box e maschere segmentate.
- **SAM (Segment Anything Model)**: Un modello che segmenta qualsiasi oggetto senza training specifico, utilizzando un Vision Transformer e prompt per generare maschere di segmentazione dettagliate e precise.
...
Inizialmente, il progetto prevedeva l'utilizzo di un flusso video standard per la scansione degli oggetti, seguendo una pipeline che includeva l'estrazione dei frame, la determinazione delle pose della telecamera con **COLMAP**, la ricostruzione fotorealistica con **NeRF**, la generazione della mesh finale e il confronto con il file `.STEP`.

Tuttavia, dopo mesi di ricerca e sviluppo, il team ha riscontrato che i risultati ottenuti da questa metodologia erano imprecisi e non idonei a garantire l'accuratezza richiesta per il controllo qualità. Le problematiche principali includevano sensibilità all’illuminazione, alta precisione di registrazione da parte dell'utente finale, impossibile da ottenere senza del training. Tra le tecnologie testate, **DepthFusion**, che usa il deep learning per combinare dati di profondità , si è rivelata inefficace a causa di problemi di velocità e precisione. Anche le sperimentazioni con NeRF, un modello neurale che ricostruisce una scena 3D da foto 2D, hanno mostrato limiti pratici in un contesto industriale a causa degli eccessivi tempi di elaborazione. Il team ha notato che una mesh triangolare compatta può essere generata dal NeRF output utilizzando un processo di **Marching Cubes**.
...
Il software **CloudCompare** è stato utilizzato per confrontare e analizzare modelli 3D da diverse fonti, come scansioni LiDAR, e file CAD, identificando differenze e visualizzandole tramite mappe di colore per evidenziare le aree con deviazioni.
